package net.foulest.apollo.tick;
...
public class TickManager implements Runnable {
    ...
    public void start() {
        ...
        task = Bukkit.getScheduler().runTaskTimer(Apollo.INSTANCE.getPlugin(), this, 0L, 1L);
    }
    ...
    public void run() {
        ...
        Apollo.INSTANCE.getPlayerDataManager().getEntries().parallelStream().forEach(playerData -> {
            ...
            transaction:
            {
                 ...
                 * This can be used later on for a ping spoof check or for getting the player's ping with ease on response.
                 */
            ...
            keepalive:
            {
                ...
                /*
                 * We don't need to do any fancy randomization for keep alives since they're much simpler
                 * as operations in comparison to keep alives, and no matter how many of them we send there won't be
